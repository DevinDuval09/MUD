...
class Server(object):
    ...
    * self.room: one of 0, 1, 2, 3. This signifies which "room" the client is in,
    ...
    player = Character(
        "Ghenghiz Cohen", rooms_dict[0], STR=5, DEX=5, INT=5, CON=5, CHA=5
    )
    player.proficiency_skills["sword"] = 3
    player.proficiency_skills["mace"] = 2
    character_dict = {"Ghengiz Cohen": player, "training dummy": training_dummy}
    ...
    def greet(self):
        ...
        self.output_buffer = "Hello {}! Welcome to {}! {}".format(
            self.player.name, self.game_name, self.player.room.description()
        )
    ...
    def kill_player(self, player: Character) -> str:
        ...
        logger.info(
            f"{player.name} inventory: {[item._description for item in player.inventory]}"
        )
        logger.info(
            f"{player.name} equipment: {[item._description for item in player.equipment.values() if bool(item) is True]}"
        )
    ...
    def inflict_damage(self, attacker: Character, defender: Character) -> str:
        if not attacker.equipment["main hand"]:
            ...
            damage = roll_d10() + (
                attacker._Character__get_stat("strength") // 3
            )  # damage roll
    ...
    def combat_exchange(
        self, attacker: Character, defender: Character, to_hit_rolls: dict
    ) -> str:
        logger.info("to hit stats for attacker: %s" % to_hit_rolls[attacker])
        logger.info(
            "armor calc for defender: %s"
            % {
                defender._Character__get_stat("armor")
                + defender._Character__get_stat("dexterity") // 3
            }
        )
        if to_hit_rolls[attacker] >= (
            defender._Character__get_stat("armor")
            + (defender._Character__get_stat("dexterity") // 3)
        ):  # defender defense calc
            message = self.inflict_damage(attacker, defender)
        else:
            message = (
                f"{attacker.name} attack bounces off the armor of {defender.name}."
            )
        return message
        ...
        while attacker.current_health > 0 and defender.current_health > 0:
            ...
            for character in [attacker, defender]:
                ...
                if character.equipment["main hand"]:
                    ...
                    hit_bonus[character] = character.proficiency_skills.get(
                        weapon.associated_skill, 0
                    )
                else:
                    hit_bonus[character] = 0
            for character in [attacker, defender]:
                init_rolls[character] = roll_d20() + character._Character__get_stat(
                    "dexterity"
                )
            for character in [attacker, defender]:
            ...
            message = self.combat_exchange(
                players_turn_order[0], players_turn_order[1], to_hit_rolls
            )
            ...
            message = self.combat_exchange(
                players_turn_order[1], players_turn_order[0], to_hit_rolls
            )
            self.output_buffer = message
            self.push_output()
        ...
        if client_input == "quit":
            ...
            try:
                ...
                if command == "attack":
                    ...
                    if key is not None:
                        self.output_buffer = getattr(self.player, command)(
                            rooms_dict[key]
                        )
                    else:
                ...
                try:
                    ...
                    self.output_buffer = (
                        "The ancient and powerful magic of AttributeErrors and text managment "
