...
class MongoConnection:
    ...
    def __init__(self, host='127.0.0.1', port=27017, db_name=None, related_collections=[]):
        '''
    ...
    def _create_dict(update=True, **kwargs):
        ...
        if update:
            return {'$set': {kw : arg for (kw, arg) in kwargs.items() if arg is not None}}
        return {kw : arg for (kw, arg) in kwargs.items() if arg is not None}

mongo = MongoConnection(db_name='Realms_Mud', related_collections=['Characters','Items', 'Rooms'])